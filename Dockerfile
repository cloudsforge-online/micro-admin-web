...
#
# Vite copies `publicDir` into `dist` during the build, so the favicons only reach the image if
# they are in the build context. `micro-web-template`'s Dockerfile copies tsconfig, vite.config,
# index.html and src — and not public — so every frontend cut from it builds an image whose
# `dist/` has no favicon in it, while `test/brand-chrome.test.ts` passes because it reads the
# SOURCE tree. The icons are wired, committed, tested, and absent from the artefact that is
# actually served: the tab still shows the browser's blank-page glyph, which is §3.3e all over
# again one layer down. Reported to `micro-web-template`; corrected here.
#
# `test/brand-chrome.test.ts` in this repository reads the Dockerfile and fails without this line,
# so it cannot be lost in a later edit.
# ══════════════════════════════════════════════════════════════════════════════════════════════
COPY public ./public
