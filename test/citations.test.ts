 ...
 *
 * This bundle is documented by citation: over five hundred paths into `admin-api/src/server.ts`,
 * `foresight/src/server.ts`, `identity/src/server.ts`, `ledger/src/reconcile.ts` and more. A citation
 ...
 * to edit. This repository had already paid for it once: when admin-api's engagement routes landed,
 * all twenty-three entries in `ADMIN_ROUTES` shifted and had to be corrected by hand for an edit
 * that changed nothing this console calls. micro-trade then did the same to micro-trade-web by
 * inserting seven lines near its imports, and every route below moved, so every citation into it
 * broke while nothing in the citing repository was wrong — and nothing runs this
...
/** Where a sibling is checked out. `micro-admin-api` and `admin-api` are the same directory. */
 ...
 * — a `${root}/src/…` in a test — and reports it as a citation to a file this repository does not
 * have. That is a false accusation, and a checker that makes them is one a reader learns to ignore.
 */
...
describe('every citation names a file that exists', () => {
  ...
  it('carries no line numbers, because a line number in another repository cannot be kept true', () => {
    // The rule, enforced rather than described. A path into micro-admin-api followed by a colon and
    // a number is a claim about a file this repository does not own and does not watch; it goes stale
