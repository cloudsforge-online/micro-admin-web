 ...
 *
 * It is not correct as a rule for the whole suite, and the support console is the reason. Its most
 * important property is a RELATION between two things that only exist once the page has rendered:
 * what the two audit queries returned, and what the screen says about what they could not contain.
 * A source-text test can assert that `CoveragePanel` is called on the empty branch. It cannot
 * assert that an operator reading the empty result is told the mirror is missing — which is the
 * whole of the requirement, and the one thing that stops this screen from lying.
 *
 ...
 *
 * Modelled on `market-web/test/dom.ts` by way of `hub-web/test/dom.ts`, which is the version of
 * this harness that has been run against real components. It is a copy rather than a shared
 * package because a test harness in `@cloudsforge/ui` would be a fourth thing every frontend has
 * to upgrade in step; when a fifth surface needs it, that is the moment to extract it.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 *
...
export interface MountOptions {
  ...
  /** What `window.confirm` answers. happy-dom does not implement it. */
  confirm?: boolean
   ...
   * a `useState` initialiser is INVOKED TWICE, so a latch proven only here has never been run the
   * way the app runs it.
   *
   ...
   * in the previous sweep shipped a StrictMode assertion that had never once exercised StrictMode.
   */
  strict?: boolean
}
...
export interface Screen {
   ...
   *
   * Nothing on the support console navigates — it is a read-only screen and `src/pages/support.tsx`
   * records that as a decision rather than an omission. Which makes this array's usual value the
   * EMPTY one, and asserting that it stays empty is a real assertion on this surface: a console
   * that grew a shortcut around the approval queue would show up here as a navigation.
   *
   * Captured rather than executed. The trap this is written against is real and recent: a test
   * pinning the SSO callback compared a URL with a copy of itself, and therefore could never
   * fail. Every assertion on this array must compare it against a value built from the SCENARIO'S
   * OWN inputs, never against something read back off the page.
   */
   ...
   *
   * The support console reads a user's whole history into memory. Nothing on it should write any
   * of that to storage, and a search of the rendered page cannot establish that either way.
   */
 ...
 * and sign-in forms is labelled with `<label for>` , and without it
...
export async function mount(element: ReactElement, options: MountOptions = {}): Promise<Screen> {
  const url = options.url ?? 'https://admin.cloudsforge.online/'
  ...
  // `location.assign` is the only way this bundle leaves a page (`lib/api.ts` signIn/signOut).
  // Recorded rather than followed; see the note on `Screen.navigations`.
